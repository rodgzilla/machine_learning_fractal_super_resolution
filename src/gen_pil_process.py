...
def multi_render(nb_renders, prefix):
    pool = multiprocessing.Pool(multiprocessing.cpu_count())
    pool.map(render_image, ['%sfrac_%06d.bmp' % (prefix, render_id) for render_id in range(nb_renders)])

if __name__ == '__main__':
    if len(sys.argv) != 3:
        print('Usage %s nb_renders prefix' % sys.argv[0], file = sys.stderr)
        exit()
    multi_render(int(sys.argv[1]), sys.argv[2])
